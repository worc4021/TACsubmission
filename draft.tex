...
This paper considers the use of parametrised sets for model uncertainties, which were recently introduced in~\cite{Schaich:CDC:2015,Schaich:NMPC:2015}. As parameters we consider the state and the control input; this allows linearisation errors and multiplicative uncertainties to be modelled as additive disturbances.
%
The paper's contribution is to develop
%extends~\cite{Schaich:CDC:2015} by developing 
the analytical and computational tools that are needed to characterise and determine invariant and controllable sets for systems with parametrised uncertainty sets, and to implement them in a closed loop RMPC strategy. To this end, we provide necessary and sufficient conditions for the property, introduced in~\cite{Schaich:CDC:2015}, of \emph{parametric convexity}, which is essential in order to obtain convex MRPI and controllable sets, and we derive a bound on the number of iterations needed to compute the MRPI set. The current paper also develops and extends the min-max RMPC approach introduced in~\cite{Schaich:NMPC:2015} by considering the case of degenerate constraints.
...
suboptimal. 
%
Explicit RMPC algorithms (e.g.~\cite{Bemporad:2003,Diehl:2004}) optimise the worst case predicted performance for each admissible model state offline and
...
%
In this paper we use an active-set approach based on that presented in~\cite{Buerger:ACC,Buerger:IJRNC,Schaich:NMPC:2015}.
