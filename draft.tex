...
\ \forall i \in\mathcal{I}\} .
\end{align*}
Here $\zeta_i(x)$ is the solution of a mpLP and hence is a continuous piecewise affine function of $x$~\cite{Gal:1995}. It follows that $S\ominus\mathcal{V}(S)$ is a convex polyhedral set.
%
...
\textcolor{red}{Given $\mathcal{R}_{k-1} = \{ x : \xi_{i,k-1} x \leq 1 \ \forall i \in \mathcal{I}_{k-1}\}$, we can compute $\mathcal{R}_k$ using (\ref{eq:Rk:iteration}) and (\ref{eq:definition:PWA:polytopic:set:vertices}) as
...
$\mathcal{X}_0 \gets \mathcal{X}$, \textcolor{red}{$\mathcal{R}_0 \gets \mathcal{X}$}, $k\gets 1$.\\
% \begin{tabular}[t]{@{}l@{}}
...
for notational convenience we write $\mathcal E=B_1(0)$). Then Proposition~\ref{prop:Dk:iteration} implies $\mathcal{D}_k=\Psi^{-k}\mathcal{R}_k\supseteq\rho^{-k}r_1\mathcal{E}$. 
