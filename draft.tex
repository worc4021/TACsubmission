...
\end{proof}
%
\todo{Manuel:generic}{It is worth pointing out that Corollary~\ref{thm:p:convexity:PWA:set:constant:num:verts} can be reformulated in a numerically helpful way:
%
\begin{cor}\label{thm:combinatorical:equivalence:alternative}
The generic set~$\mathcal W(p)$ defined by~\eqref{eq:definition:PWA:polytopic:set:general} is parametrically convex if and only if, it is combinatorially equivalent for any $p_1,p_2\in\mathbb R^d$ almost everywhere, i.e. $\mathcal W(p_1)\cong\mathcal W(p_2)$.
\end{cor}
%
\begin{proof}
...
%
It follows from the definition that $\mathcal X_k\subseteq\mathcal X_0=\mathcal X$ for all $k\in\mathbb N$ that $\mathcal V(x)\subseteq c_3\mathcal E$ for all $x\in\mathcal X$ implies that the containment holds for all $x\in\mathcal X_k$.
%
We argue inductively: Clearly every $x$ such that $\{\rho x\}\oplus c_3\mathcal E\subseteq c_1\mathcal E$ also satisfies $\{\Psi x\}\oplus \mathcal V(x)\subseteq c_3\mathcal E \subseteq \mathcal X_0$, but the first condition is equivalent to $x\in\rho^{-1}(c_1\mathcal E\ominus c_3\mathcal E)=\rho^{-1}(c_1-c_3)\mathcal E$.
%
It follows that the ball of radius $\rho^{-1}(c_1-c_3)$ is contained in $\mathcal D_1^\prime$, we continue by determining the largest ball contained in $\mathcal D_2^\prime$ in the same fashion.
%
All points that satisfy $\{\rho x\}\oplus c_3\mathcal E \subseteq \rho^{-1}(c_1-c_3)\mathcal E$ also satisfy $\{\Psi x\}\oplus\mathcal V(x)\subseteq \mathcal D_2^\prime$ and so on.
%
Recursively we derive a ball for all subsequent $k$ such that $r_k\mathcal E\subseteq\mathcal D_k^\prime$, this radius is given by $r_k = \rho^{-k}(c_1-c_3\sum_{l=0}^{k-1}\rho^l) = \rho^{-k}(c_1-c_3\frac{1-\rho^k}{1-\rho})$.
%
Since $\mathcal X_k$ is defined as an intersection with the previous iterate the set can not change as soon as the inner ball has \emph{outgrown} any ball that contains $\mathcal X_k$.
%
We know that $\mathcal X_k\subseteq\mathcal P_n\subseteq c_2\mathcal E$ for all $k\geq n$ so that we have that $\mathcal X_k = \mathcal X_{k+1}=\mathcal X^\infty$ no later than the smallest $k\in\mathbb N$ such that $k\geq n$ and $c_2<r_k$, bound~\eqref{eq:iteration:cbound} follows from:
\[
