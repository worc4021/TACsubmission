...
%Let $X\subseteq\mathbb R^d, Y\subseteq\mathbb R^n$, let $\mathscr P(Y)$ denote the power set of $Y$, and
...
\]
%Then the Minkowski functional (see e.g.~\cite{Rudin:91}) yields $\mu_{\mathscr G(\mathcal W)}(\lambda p_1 + (1-\lambda)p_2,\lambda x_1+(1-\lambda)x_2)\geq1$,
...
N\mathcal W(p_2,x_2)\neq\emptyset$, 
%i.e.~that for some $\lambda \in (0,1)$
% \[
% \lambda x_1 + (1-\lambda)x_2\in\text{int}(\mathcal W(\lambda p_1 + 
% (1-\lambda)p_2) ,
% \]
%
i.e.~that there exists $\epsilon>0$ such that the ball
%full dimensional ball 
%$B_\epsilon(x ) = \{y:\| x-y\|<\epsilon\}$
$B_\epsilon(\lambda x_1 + (1-\lambda)x_2 )$
is contained in $\mathcal W(\lambda p_1 + (1-\lambda)p_2)$ for some $\lambda \in (0,1)$.
%i.e. $B_\epsilon(\lambda x_1 + (1-\lambda)x_2 )\subset \mathcal W(\lambda p_1 + (1-\lambda)p_2)$ for some $\epsilon>0$.
%
...
%Suppose that for some region $\Omega\subset\mathbb R^d$ the function $r(\cdot,x)$ is non-concave (i.e. strictly convex), 
...
%The MRPI set $\mathcal X^\infty$ defined in~\eqref{eq:definition:MRPI:set} 
...
%i.e.~$v_{l,i}^\ast(x)$, $l\in\mathbb N_[0,k]$, for $i\in\mathcal I_0$.
%
...
%  =& \bigcap_{l\in \mathbb N_{k+1}} \left( \Psi^{-l} \mathcal X_0 \ominus \underset{i\in\mathbb N_{l-1}}{\bigoplus}  \Psi^i \mathcal V(\mathcal X_{l-1})\right) \cap \mathcal X_0.
%  =& \bigcap_{0\leq l\leq k+1}\left( \Psi^{-l} \mathcal X_0 \ominus \underset{1 \leq i\leq l-1}{\bigoplus} \Psi^i \mathcal V(\mathcal X_{l-1})\right).
\end{split}\end{equation}
%
...
%\bigcap_{0\leq l\leq n-1} \Psi^{-l} B
\]
%
...
%$\mathcal E(r_1)\subseteq\mathcal X_0$ 
...
%
Then, for any $l\geq n$ such that %$\mathcal X_p\subseteq 
%$r_3 \mathcal E\subseteq (\rho^{-l} r_1 - (1-\rho)^{-1} r_2) \mathcal E$, 
...
%intersections with $D_l$ will not change $\mathcal X_l$.
%
