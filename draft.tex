...
algorithm in section~\ref{sec:example:II} and section~\ref{sec:conclusion} provides brief conclusions.
%
%Section~\ref{sec:conclusion} provides brief conclusions and proofs are given in the Appendix. 

\emph{Notation:} 
Throughout this paper we denote sequences of integers $\mathbb N_m = \{1,\ldots,m\}$ for $m\geq 1$, $\mathbb N_{[n,m]} = \{n,\ldots,m\}$ for $n\leq m$, and $\mathbb N = \mathbb N_\infty$.
%
...
%
The image of a set $\mathcal C\subseteq X$ under the map $f:X\mapsto Y$ is denoted $f\vert_{\mathcal C} = \{y\in Y:\exists x\in\mathcal C,\;y=f(x)\}$ and the pre-image is $f^{-1}\vert_{\mathcal C} = \{x\in X : f(x)\in \mathcal C\}$; for $f(x) = Ax$ we write $f\vert_{\mathcal{C}} = A\mathcal{C}$ and $f^{-1}\vert_{\mathcal{C}} = A^{-1}\mathcal{C}$.
%
...
Let $\mathcal W:Y\rightarrow \mathscr P(Z)$, where $Y\ni p\mapsto \mathcal W(p) \subset Z$, be a continuous point-to-set map. The map $\mathcal W$ is \emph{parametrically convex} if it satisfies
%
...
\]
where $\mathcal V(x)$ is a continuous point-to-set map. The state $x$ is subject to the constraint $x\in\mathcal{X}$ and $x^+ = \Psi x$ is assumed to be an asymptotically stable system. 
%
...
\end{equation} 
for some scalar $\varrho\in (0,1)$, so that $\Psi\mathscr B_P(r) \subseteq \varrho\mathscr B_P(r)$ and $\Psi^{-1}\mathscr B_P(r) \supseteq \varrho^{-1}\mathscr B_P(r)$ for all $r>0$. 
...
$\varrho \leq \rho(\Psi) < 1$
where $\rho(\Psi)$ is the spectral radius of $\Psi$ (see e.g.~\cite{Boyd:94}). 

\begin{thm}\label{thm:finite:MRPI:set:state:dependable}
Let $\rho(\Psi) < 1$ and let $\mathcal{X}\subseteq\mathcal{P}$, $\mathcal{P}=\{x:\Gamma x\leq{\bf{1}}, {-\Gamma} x\leq{\bf{1}}\}$, where $(\Psi,\Gamma)$ is an observable pair.
%
...
%
\begin{proof}
From~\eqref{eq:Xk:iteration} and Proposition~\ref{prop:Dk:iteration} it is clear that $\mathcal X^\infty$ is empty if $\mathcal{R}_k =\emptyset$ for some $k$.
%
For the remainder of the proof we assume that $\mathcal{R}^\infty \neq \emptyset$ exists such that $\mathcal{R}_k \supseteq \mathcal{R}^\infty$ for all $k\in\mathbb N$.
%
...
% we have
\[%\begin{multline*}
\mathcal D_k = \Psi^{-k}\mathcal R_k\supseteq \Psi^{-k}\mathcal R^\infty\supseteq \Psi^{-k}\mathscr B_P(r_1)
%\left\{x:\norm{\Psi^k x}_P\leq r_1\right\}\\ 
% \supseteq\mathscr B_P\left(\left((1-\delta)^{-\frac{1}{2}}\right)^k r_1\right) 
%\supseteq
\supseteq\mathscr B_P\left(\varrho^{-k} r_1\right).
\]%\end{multline*}
%
Next define $r_2$ so that $\mathscr B_P(r_2)\supseteq \mathcal X_n$,
%
then for all $k > n$ such that %$\left((1-\delta)^{-\frac{1}{2}}\right)^k r_1\geq r_2$ 
$\varrho^{-k} r_1 \geq r_2$
...
\log_{\varrho}(r_1/r_2)\bigr\rceil -1}\Bigr\},
%\frac{\log(r_2/r_1)}{\log(1/\rho)}\right\rfloor} \right\},
\end{equation}
and it follows that $\mathcal{X}^\infty$ is a bounded polytopic set.
\end{proof}

Although the bound (\ref{eq:iteration:bound}) on the number of iterations required by Algorithm~\ref{alg:MRPI:set} is necessarily finite, $r_1$ and $r_2$ cannot in general be determined \textit{a priori}. 
%
...
\textcolor{blue}{\[
c_1 = \max_x x^T P x \ \ \text{subject to} \ \  \Xi x\leq {\bf{1}},
\]}
% \[
% c_1 = \min_j\bigl\{\min_x x^T P x \ \ \text{subject to} \ \  \Xi x\leq {\bf{1}},\ \xi_j x = 1\bigr\}
% \]
...
performed which approximately is linear. **LABEL AXES**}
