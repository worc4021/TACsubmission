...
%
In this section we define \emph{parametric convexity} and summarise relevant results together with the central ideas on particular set-valued maps.
%
For a detailed discussion on parametric convexity we refer to~\cite{Schaich:2016comb}.
%
...
%
In the following we present an equivalent characterisation of parametric convexity based on the \emph{graph} of the set-valued map.
...
%
The next statement makes~\eqref{eq:graph:def:p:convexity} more precise.
...
then $\mathcal W$ is parametrically convex iff the function $r$ is concave in $p\in\mathbb R^d$.
%
\end{cor}
%
% \begin{proof}
...
%
This property fixes the combinatorial structure of $\mathcal W(\tilde p)$ for $\tilde p$ in a neighbourhood around $p$, in fact the combinatorial structure has to be fixed for all admissible parameters~$p\in Y$ as summarised in the next statement.
\end{rem}
%
\begin{cor}\label{thm:combinatorical:equivalence:alternative}
The generic set~$\mathcal W(p)$ defined by~\eqref{eq:definition:PWA:polytopic:set:general} is parametrically convex if and only if, it is combinatorially equivalent for any $p_1,p_2\in Y$ almost everywhere, i.e. $\mathcal W(p_1)\cong\mathcal W(p_2)$.
\end{cor}
% %
% \begin{proof}
...
%
This means, similar to simplicity, while the position and orientation of hyperplanes supporting the polyhedron may change, the combination of which hyperplanes support each vertex or ray can not change.
%
In particular, index set~$\mathcal I_\kappa$ defining the vertex 
%
\begin{equation}\label{eq:def:v:kappa}
  v_\kappa(p)=\{z\in\mathbb R^n: a_i z = \max_k\{b_{i,k} + c_{i,k}p\}\;i\in\mathcal I_\kappa\}
\end{equation}
%
also defines a continuous selection of extremal points of $\mathscr G(\mathcal W)$ in the sense of~\cite{Finzel2000}.
%
Knowledge of all index sets~$\mathcal I_\kappa$ therefore allows us to have the equivalent description~$\mathcal W(p)=\conv_\kappa\{v_\kappa(p)\}$, which will allow us to perform numerical calculations.
%
%
% \begin{rem}
...
In order to determine MRPI sets we need to be able to calculate the Pontryagin difference between set-valued maps; hence we introduce the following.
%
...
%
In the next section we will show that the parametric Pontryagin difference between a polyhedral set and a piecewise affine polytopic parametric set is itself polyhedral.
%
%
%
%
...
%
where $v_j(x)$ is defined by the $j$-th active index set~$\mathcal A_j$ as in~\eqref{eq:def:v:kappa}.
%
Therefore $v_j(x)$ is necessarily piecewise affine in $x$ for all $j\in\mathbb{N}_q$ and we will see that it is unnecessary to % evaluate $\Theta_j$ and $\theta_j$ explicitly.
have its explicit representation.
%
...
%
If this is not possible a projection can be used to perform the parametric Pontryagin difference, see~\cite{Schaich:2016comb}.
% \textcolor{red}{%
Either way, the resulting set can be expressed $S\ominus \mathcal{V}(S) = \{ x : h_i^\prime x \leq 1 \ \forall i \in \mathcal{I}^\prime \}$.%
% }%
\end{rem}
%
...
and\\
$\mathcal{V}(x) = \conv_{j\in\mathbb{N}_q}\{v_j(x)\}$\\
equivalently determine all $q$ $\mathcal A_j$.
\end{tabular}
\\
...
%
To show that algorithm~\ref{alg:MRPI:set} terminates in a finite number of iterations we will use norm balls that contain and are contained in the sets used in the algorithm.
%
For this let $\norm{\cdot}$ denote any norm in $\mathbb R^n$ and let $\mathscr B(r)$ be the norm ball of radius $r$, i.e.~$\mathscr B(r)=\{x\in\mathbb R^n:\norm{x}\leq r\}$.
%
\begin{thm}\label{thm:norm:balls:set:operations}
...
\end{proof}
%
Since we assume the system~$x^+=\Psi x$ to be asymptotically stable, in particular its spectral radius to be strictly less than one $\rho(\Psi)<1$, the converse Lyapunov theorem states that there exists one positive definite $P>0$ such that the Lyapunov equation $\Psi^TP\Psi-P+Q=0$ holds for any positive definite $Q>0$, this matrix is given by $P=\sum_{k\geq0}(\Psi^k)^TQ\Psi^k$.
%
...
We will now show that Algorithm~\ref{alg:MRPI:set} is guaranteed to terminate after a finite number of iterations if:
%
...
%
The finite determinability of the MRPI set $\mathcal X^\infty$ implies that there exists a finite number, $M$, such that $\mathcal X^\infty = \mathcal X_M$, and hence  $\mathcal{X}^\infty$ is defined by a finite set of linear inequalities whenever $\mathcal X^\infty\neq \emptyset$.
%
...
Let $\rho < 1$, where $\rho$ is the spectral radius of $\Psi$, and let $\mathcal{X}\subseteq\mathcal{P}$, $\mathcal{P}=\{x:\Gamma x\leq{\bf{1}}\wedge -\Gamma x\leq{\bf{1}}\}$, where $(\Psi,\Gamma)$ is an observable pair.
%
...
To see that $\mathcal D_k$ grows exponentially, let $r_1$ denote the radius of the largest $P$-ball centred at the origin that is contained in $\mathcal R^\infty$, i.e. $\mathscr B_P(r_1) = \{x:x^TPx\leq r_1^2\}\subseteq\mathcal R^\infty$. 
%
Then Proposition~\ref{prop:Dk:iteration} in conjunction with Theorem~\ref{thm:norm:balls:set:operations} yield that 
\begin{multline}
  \mathcal D_k = \Psi^{-k}\mathcal R_k\supseteq \Psi^{-k}\mathcal R^\infty\supseteq \left\{x:\norm{\Psi^{-k}x}_P\leq r_1\right\}\supseteq\\ 
\mathscr B_P\left(\left((1-\delta)^{-\frac{1}{2}}\right)^k r_1\right)% \supseteq\mathscr B_P\left(\rho^{-k} r_1\right).
\end{multline}
%
To complete the proof let $r_2$ denote the largest $P$-ball containing $\mathcal X_n$, i.e. $\mathcal X_n\subseteq\mathscr B_P(r_2)$.
%
Then for $k\geq n$ such that $\left((1-\delta)^{-\frac{1}{2}}\right)^k r_1\geq r_2$ we have that $\mathcal D_k\supseteq\mathcal X_n\supseteq\mathcal X_{k-1}$ so that $\mathcal X_k = \mathcal X_{k-1}$, which concludes the proof.
...
\begin{proof}
\todo{Manuel:proof:rewritten}{
Iteration~\eqref{eq:Xk:iteration} can be defined as $\mathcal X_{k+1} = \mathcal X_k\cap\mathcal D_{k+1}^\prime$ with $\mathcal X_0=\mathcal X$ where
$\mathcal D_k^\prime$ is defined by 
%
$$
\mathcal D_k^\prime = \{x\in\mathcal X_{k-1}:\{\Psi x\}\oplus\mathcal V(x)\subseteq \mathcal D_{k-1}^\prime\}
$$
%
and initialised as $\mathcal D_0^\prime=\mathcal X$.
%
It follows from the definition that $\mathcal X_k\subseteq\mathcal X_0=\mathcal X$ for all $k\in\mathbb N$ that $\mathcal V(x)\subseteq \mathscr B_P(c_3)$ for all $x\in\mathcal X$ implies that the containment holds for all $x\in\mathcal X_k$.
%
We argue inductively: Clearly every $x$ such that $\{\rho x\}\oplus \mathscr B_P(c_3)\subseteq\mathscr B_P(c_1)$ also satisfies $\{\Psi x\}\oplus \mathcal V(x)\subseteq\mathscr B_P(c_3) \subseteq \mathcal X_0$, but the first condition is equivalent to $x\in\rho^{-1}(\mathscr B_P(c_1)\ominus\mathscr B_P(c_3))=\rho^{-1}\mathscr B_P(c_1-c_3)$.
%
It follows that the $P$-ball of radius $\rho^{-1}(c_1-c_3)$ is contained in $\mathcal D_1^\prime$, we continue by determining the largest $P$-ball contained in $\mathcal D_2^\prime$ in the same fashion.
%
All points that satisfy $\{\rho x\}\oplus\mathscr B_P(c_3) \subseteq \rho^{-1}\mathscr B_P(c_1-c_3)$ also satisfy $\{\Psi x\}\oplus\mathcal V(x)\subseteq \mathcal D_2^\prime$ and so on.
%
Recursively we derive a $P$-ball for all subsequent~$k$ such that $\mathscr B_P(r_k)\subseteq\mathcal D_k^\prime$, this radius is given by $r_k = \rho^{-k}(c_1-c_3\sum_{l=0}^{k-1}\rho^l) = \rho^{-k}(c_1-c_3\frac{1-\rho^k}{1-\rho})$.
%
Since $\mathcal X_k$ is defined as an intersection with the previous iterate the set can not change as soon as the inner $P$-ball has \emph{outgrown} any $P$-ball that contains $\mathcal X_k$.
%
...
%
\[
\rho^k(c_2-\frac{c_3}{1-\rho})<c_1-\frac{c_3}{1-\rho}
\]
%
To preserve the orientation of the inequality we require $c_2-c_3(1-\rho)^{-1}>0$ and in order to obtain a positive argument in the logarithm we also require $c_1-c_3(1-\rho)^{-1}>0$.
%
...
and for $c_3$ we can use a projection and the known combinatorial structure of~$\mathcal W(x)$ in the sense of~\eqref{eq:def:v:kappa}.
%
First project the set 
%
$$
  \mathcal T = \{(\tau,x):\Xi x\leq {\bf{1}}\wedge b_{i,k}+c_{i,k}x\leq \tau_i\forall i\in\mathbb N_m,k\}
$$
onto $\mathbb R^m$, thereby all vertices of the projection are such that there exists a state~$\tilde x\in\mathcal X$ such that $\mathcal W(\tilde x)=\conv_\kappa\{\{w:a_i w = \tau_i\, i\in\mathcal A_\kappa\}\}$.
%
Therefore maximising over all vertices of the projection leads to the desired radius
\[
c_3 = \max_{\tau\in\text{vert}(\pi_m(\mathcal T))}\max_\kappa w_\kappa^TPw_\kappa, w_\kappa = \{w:a_i w = \tau_i\, i\in\mathcal A_\kappa\}
\]
\end{rem}
