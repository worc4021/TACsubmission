...
$\mathcal W(x)$ defined by~\eqref{eq:definition:PWA:polytopic:set:general}. 
%
...
\begin{multline}
  \bar\sigma({\bf{c}})\norm{x}+\norm{{\bf{b}}}
  \geq\bar\sigma({\bf{c}}_{k^\ast})\norm{x}+\norm{{\bf{b}}_{k^\ast}}\\
  =\max_k\{\bar\sigma({\bf{c}}_k)\norm{x}+\norm{{\bf{b}}_k}\}
  \geq\norm{\max_k\{{\bf{c}}_{\mathscr{A}_i,k} x + {\bf{b}}_{{\mathscr{A}_i},k}\}}\\ 
  =\norm{{\bf{a}}_{\mathscr{A}_i}v_i} \geq\underline\sigma({\bf{a}}_{\mathscr A_i})\norm{v_i}
  \geq\underline\sigma({\bf{a}})\norm{v_i}.
\end{multline}
%
...
exact bounds on intermediate iterates, hence $r_2\leq\frac{\bar\sigma({\bf{c}})r_3+\norm{{\bf{b}}}}
{\underline\sigma({\bf{a}})}$ can be used as an estimate for the asymptotic behaviour of $\mathcal V(x)$
in the iteration.
%
...
In this section we discuss how multi-parametric quadratic programs (mpQP) can be solved efficiently with an active-set 
solver (see e.g.~\cite{Fletcher:1987}).
%
...
\end{equation}
%
It can be shown (see~\cite{Schaich:NMPC:2015}) that the solution of~\eqref{app:kkt:conditions} has the structure
%
