...
\begin{equation}\label{eq:stage:constraints:in:x:and:u}
  \mathcal Z_i = \{(x,u)\in\mathcal X\times \mathcal{U}: Ax + Bu + Dw \in\mathcal X_{i-1}\;\forall w\in\mathcal W(x,u)\}
\end{equation}
%
and its projection
%
...
\begin{equation}\label{eq:definition:disturbance:set:explicit}
  \mathcal W(x,u) = \bigl\{w:G_i w\leq\max_{k}\{H_{i,k}^x x + H_{i,k}^uu + h_{i,k}\}\; \forall i\in\mathcal{I}_{\mathcal{W}}\bigr\}.
\end{equation}
%
...
\end{defi}
%
Notice that Definition~\ref{def:parametric:convexity} does not require convexity of~$\mathcal W(p)$. However, we will only consider maps~$\mathcal W$ for which $\mathcal W(p)$ is convex for all $p\in Y$.
...
Let $\mathcal W: Z\rightarrow\mathscr P(Z)$ denote a point-to-set map, then the parametric Pontryagin difference $S \ominus \mathcal W(S)$ is convex for all convex $S\subseteq Z$ if and only if $\mathcal W$ is parametrically convex.
\end{thm}
% \begin{thm}\label{thm:convexity:of:pontryagin:difference}
...
This iterative procedure is summarized as follows.
%In summary we have the following iteration.
%
...
$\mathcal{X}_0 \gets \mathcal{X}$, $k\gets 1$.\\
\begin{tabular}{@{}l@{~}l@{}}
1. &
...
%
Then the MRPI set $\mathcal X^\infty$ for the system $x^+ = \Psi x + v$ subject to disturbances $v\in\mathcal{V}(x)$, where $\mathcal V(x)$ is a continuous point-to-set map, and constraints $x\in\mathcal{X}$, is either empty or else is a bounded polytopic set satisfying $\mathcal X^\infty =\mathcal X_M$ for finite $M$.
\end{thm}
%
%
\begin{proof}
From~\eqref{eq:Xk:iteration} and~\eqref{eq:Dk:iteration} it is clear that $\mathcal X^\infty$ is empty if $\mathcal{R}_k =\emptyset$ for some $k\geq 0$.
%
...
%
(ii) we show that $D_k$ grows exponentially with $k$, so that, for any bounded set $\mathcal C$, finite $M$ exists such that $D_{M+1}\supseteq \mathcal{C}$. The proof is concluded by setting 
...
for notational convenience we write $\mathcal E=B_1(0)$). Then (\ref{eq:Dk:iteration}) implies $\mathcal{D}_k=\Psi^{-k}\mathcal{R}_k\supseteq\rho^{-k}r_1\mathcal{E}$. 
...
M = \max \left\{ n , \left\lceil \frac{\log(r_2/r_1)}{\log(1/\rho)}\right\rceil - 1 \right\},
\end{equation}
...
and
\[
c_3 = \max_{j\in\mathbb{N}_q} \max_x \|\Theta_jx + \theta_j\|^2 \ \  \text{subject to} \ \ \Xi x \leq {\bf 1} ,
\]
