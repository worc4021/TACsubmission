...
%% Ticket list
\openTickets{Manuel:wlog,Manuel:invertability:assumption,Mark:log:bounds,Mark:proof:cor15,Manuel:proof:rewritten}
...
%
It follows from the definition that $\mathcal X_k\subseteq\mathcal X_0=\mathcal X$ for all $k\in\mathbb N$.
%
Due to the fact that we chose~$c_3$ such that $\mathcal V(x)\subseteq c_3\mathcal E$ for all $x\in\mathcal X$ this also holds for all $x\in\mathcal X_k$.
%
...
\[
\D_k^\prime = \bigl\{ x\in\X_{k-1} : \{\Psi x\} \oplus \V(x) \subseteq \D_{k-1}^\prime\}
\]
